...
### **2.3. What makes stochastic models stochastic?**
...
#### **2.3.1. An illustration of sampling error using OLS regression as an example**
...
Now, let's say we take a random sample of size 7 out of this population, compute the *sample* OLS line and plotted that on top of the population (sample in black, population in gray):
```{r, echo=FALSE}
par(mar=c(3, 2, 3, 1))
plot_se_ols(7)
``` 
...
See how every time we *repeat* the random sampling the OLS line changes a bit? That's called *variation upon replication*; it's a fundamental fact of working with random variables and the source of *sampling error*. Because we're computing statistics based on *samples*, and our results would *vary upon replication*, it's necessary to attach a measure of the size of that variability to every sample statistic we compute. Those are the standard errors we see in any inferential statistical analysis, and the basis of confidence and prediction intervals for sample statistics (like the sample mean, variance, covariance, $r^2$, etc.).
